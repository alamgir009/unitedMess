...
const ActivitySkeleton = () => (
    <div className="flex items-center gap-3 sm:gap-4 p-2.5 sm:p-3 animate-pulse">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-muted shrink-0" />
        ...
        <div className="group flex items-center justify-between px-3.5 py-2.5 sm:px-5 sm:py-3.5 hover:bg-muted/40 transition-[background-color] duration-150 ease-out cursor-default">
            <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
                ...
                <div className="min-w-0">
                    <p className="text-body font-semibold text-foreground truncate group-hover:text-primary transition-colors duration-150">
                        {activity.title}
                    </p>
            ...
            <div className="text-right shrink-0 ml-3">
                <p className="text-body font-bold text-foreground tabular-nums">{activity.amount}</p>
                <p className="text-caption text-muted-foreground mt-0.5 whitespace-nowrap">
...
const RecentActivityWidget = memo(function RecentActivityWidget({ activities = [], isLoading }) {
    ...
    if (isLoading) {
        return (
            <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-4 sm:p-6 flex flex-col h-full">
        ...
        <div className="bg-card border border-border/50 rounded-2xl shadow-sm hover:shadow-md transition-[box-shadow] duration-200 ease-out flex flex-col h-full overflow-hidden">
            <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-6 sm:pb-4 border-b border-border/50 flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-1 text-caption font-semibold text-primary hover:text-primary/80 transition-colors group"
                >
                    View All
                ...
                {activities.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 sm:py-14 gap-3 text-muted-foreground">
                        ...
                        <div className="text-center">
                            <p className="font-semibold text-body text-foreground">No activity yet</p>
                            <p className="text-caption mt-0.5">Your meals and market entries will appear here</p>
                        </div>
                        <button
                            onClick={() => navigate('/meals')}
                            className="mt-2 text-caption font-bold text-primary hover:text-primary/80 underline underline-offset-2"
                        >
                            Log your first meal
                        </button>
                    </div>
            ...
            {activities.length > 0 && (
                <div className="px-4 py-2.5 sm:px-6 sm:py-3 border-t border-border/40 bg-muted/20">
