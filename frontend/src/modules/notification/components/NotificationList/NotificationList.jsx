...
const listItemTransition = { duration: 0.2, ease: [0.16, 1, 0.3, 1] };
...
const ErrorState = ({ error, onRetry }) => (
    <div className="flex flex-col items-center justify-center py-14 px-6 text-center">
        <div className="w-14 h-14 rounded-full mb-4 shadow-inner bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/30 flex items-center justify-center">
            <AlertCircle className="w-5 h-5 text-red-400" />
        </div>
        <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Failed to load</h4>
        ...
        <button
            onClick={onRetry}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 hover:bg-blue-100 dark:hover:bg-blue-950/70 transition-all"
        >
...
const GroupLabel = ({ label }) => (
    <div className="px-4 py-2 sticky top-0 z-[1] bg-slate-50/90 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-800/60 backdrop-blur-sm">
...
const NotificationList = ({ closeMenu, onNotificationClick }) => {
    ...
    return (
        <div className="flex flex-col h-full max-h-[75vh] sm:max-h-[600px] bg-white dark:bg-slate-900 overflow-hidden">
            <div className="sticky top-0 z-10 shrink-0 flex items-center justify-between gap-2 px-4 py-3 sm:px-5 sm:py-4 border-b border-slate-100 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm">
                <div className="flex items-center gap-2.5 min-w-0 shrink-0">
                    <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-2xl shadow-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shrink-0">
                        <Bell className="w-4 h-4 text-white" aria-hidden />
                    </div>
                    <div className="min-w-0">
                        <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-[14px] sm:text-[15px] tracking-tight truncate">
                            Notifications
                        </h3>
                        {total > 0 && (
                ...
                <AnimatePresence>
                    {unreadCount > 0 && (
                        <motion.button
                            ...
                            className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 hover:bg-blue-100 dark:hover:bg-blue-950/70 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150"
                        >
                            {markAllLoading
            ...
            <div
                ...
                aria-live="polite"
                className="overflow-y-auto overflow-x-hidden flex-1 overscroll-contain scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700 hover:scrollbar-thumb-slate-300 dark:hover:scrollbar-thumb-slate-600"
            >
                                    ...
                                    {grouped.get(groupKey).map((notif, idx) => {
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                animate={{ opacity: 1, y: 0, transition: { delay: delay * 0.03, ...listItemTransition } }}
            ...
            {items.length > 0 && (
                <div className="shrink-0 px-4 py-3 sm:px-5 sm:py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20 flex items-center justify-between">
                    <button
                        onClick={handleRefresh}
                        className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 hover:text-blue-500 dark:hover:text-blue-400 transition-all"
                    ...
                    <button
                        onClick={closeMenu}
                        className="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 transition-colors"
                    >
                        Close
                    </button>
                </div>
